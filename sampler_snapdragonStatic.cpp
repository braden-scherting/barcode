...
arma::vec PrC_snapdragon(arma::mat C, int N, int L){
  arma::vec PrC(L);
  
  for (int l=0; l<L; l++){
    PrC(l) = R::rbeta(10 + sum(C.col(l)), 10 + N - sum(C.col(l)));
  }
  return PrC;
}

arma::mat CRandomBlocked_snapdragon(arma::mat Y, arma::mat Gamma, arma::mat S, arma::mat Phi, 
                                    arma::mat C, arma::mat Zeta, arma::vec PrC, int N, int P, int L, 
  ...
  for (int i=0; i<N; i++){
    ...
    for (int b=0; b<B; b++){
      ...
      logProbs += sum(arma::log((Cstar.each_row() % PrC(blk).t()) +
        (OneMinusCstar.each_row() % (1-PrC(blk).t()))), 1);
      //   +
...
// [[Rcpp::export]]
Rcpp::List sampler_snapdragonStatic(int niter, int nthin, int nburn, Rcpp::List dims, Rcpp::List dat, 
  ...
  arma::mat Eta(L, niter); 
  arma::vec EtaNit(L, arma::fill::value(0.5));
